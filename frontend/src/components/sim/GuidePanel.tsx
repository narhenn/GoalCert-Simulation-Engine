...
export default function GuidePanel({ sim, myRole, scenarioId, onHighlightNode }: Props) {
  ...
  if (collapsed) {
    return (
      <div style={{ width: 36, background: "#0d1320", borderRight: "1px solid rgba(255,255,255,0.06)",
        display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 12, cursor: "pointer" }}
        onClick={() => setCollapsed(false)}>
        <i className="fa fa-book" style={{ color: "var(--gc-accent)", fontSize: 14 }} />
  ...
  return (
    <div style={{ width: 280, background: "#0d1320", borderRight: "1px solid rgba(255,255,255,0.06)",
      ...
      {/* Phase progress dots */}
      {guide && (
        <div style={{ padding: "8px 12px", display: "flex", gap: 3, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          {(guide.phases as string[]).map((p: string, i: number) => (
            <div key={p} title={p} style={{ flex: 1, height: 4, borderRadius: 2,
              background: i < guide.phase_index ? "var(--gc-green)" : i === guide.phase_index ? "var(--gc-accent)" : "rgba(255,255,255,0.08)",
